...
class SwimPool:
    ...
    def marginal_prices(self, eps = Decimal(0.1)):
        prices = []
        depth = self.depth()
        for i in range(self.token_count):
            lower = depth - self.__calc_depth(
                [
                    balance - (eps if j == i else 0)
                    for j, balance in enumerate(self.balances)
                ],
                depth,
            )
            upper = -depth + self.__calc_depth(
                [
                    balance + (eps if j == i else 0)
                    for j, balance in enumerate(self.balances)
                ],
            ...
            prices.append((lower + upper) / (2 * eps))
        return prices
    ...
    def add(self, input_amounts, min_mint_amount=0):
        if self.lp_supply == 0:
            ...
            if depth < min_mint_amount:
                raise Exception(
                    f"insufficent lp tokens minted: at least {min_mint_amount} requested but only {depth} would be minted"
                )
    ...
    def remove_exact_burn(self, burn_amount, output_index, min_output_amount=0):
        ...
        if output_amount < min_output_amount:
            raise Exception(
                f"insufficent output: at least {min_output_amount} requested but only {output_amount} received"
            )
    ...
    def __swap(self, is_exact_input, amounts, index, limit):
        ...
        if limit > 0:
            if is_exact_input:
                ...
                if final_amount > limit:
                    raise Exception(
                        f"maximum input amount exceeded: {final_amount} required but only {limit} authorized to be swapped"
                    )
    ...
    def __add_remove(self, is_add, amounts, limit):
        ...
        if self.__total_fee > 0:
            ...
            taxbase = [
                max(
                    self.__sub_given_order(
                        is_add, updated_balances[i], scaled_balances[i]
                    ),
                    0,
                )
                for i in range(self.token_count)
            ...
            if limit > 0:
                if is_add:
                    if lp_amount < limit:
                        raise Exception(
                            f"insufficent lp tokens minted: at least {limit} requested but only {lp_amount} would be minted"
                        )
                else:
                    if lp_amount > limit:
                        raise Exception(
    ...
    #     for i in range(self.token_count):
    #         distance += max((self.balances[i]/pool_sum - amounts[i]/amount_sum), Decimal(0))
