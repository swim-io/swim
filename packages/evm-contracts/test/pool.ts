...
describe("Pool Defi Operations", function () {
  ...
  async function testFixture() {
    const [deployer, governance, govFeeRecip, liquidityProvider, user] =
    ...
    //deploy erc20 tokens
    const tokenData = [["swimUSD", "SwimUSD", 8],["USDC", "USD Coin", 6], ["USDT", "Tether", 6]];
    const [swimUSD, usdc, usdt] = await Promise.all(tokenData.map(
      async ([symbol, name, decimals], tokenNumber) => {
    ...
    const mockTokenBridge =
      await (await ethers.getContractFactory('MockTokenBridge')).deploy(swimUSD.address);

    //deploy SwimFactory via presigned tx
    await deployer.sendTransaction({to: FACTORY_PRESIGNED.from, value: FACTORY_PRESIGNED.maxCost});
    ...
    const routing = await deployProxy(
      routingLogic,
      SALTS.routingProxy,
      [deployer.address, mockTokenBridge.address]
    );
    ...
    await pool.connect(liquidityProvider).add(tokens.map((t) => t.toAtomic(baseAmount)), 0);
  ...
  it("RemoveUniform should empty the pool and allow refilling after", async function () {
    const { pool, govFeeRecip, liquidityProvider, lpToken, swimUSD, usdc, usdt, toAtomicAmounts } = await loadFixture(testFixture);
    ...
    for (const token of tokens)
      expect(await token.balanceOf(liquidityProvider)).to.equal(0);
  });
  ...
  it("Add should return correct outputs", async function () {
    ...
    expect(await lpToken.balanceOf(user))
      .to.be.closeTo(lpToken.toAtomic("0.976045"), tolerance);
    expect(await lpToken.balanceOf(govFeeRecip))
      .to.be.closeTo(lpToken.toAtomic("0.000063"), tolerance);
  });

  it("RemoveExactBurn and RemoveExactOutput should return correct and consistent outputs", async function () {
    const setup = async () => {
      const { pool, govFeeRecip, lpToken, usdc, user, toAtomicAmounts } = await loadFixture(testFixture);
  ...
  it("Swap equals SwapExactInput exactly", async function () {
    ...
    const swapExactInput = async () => {
      const { pool, govFeeRecip, lpToken, usdc, usdt, user, toAtomicAmounts } =
  ...
  it("swapExactInput and SwapExactOutput should return correct and consistent outputs",
  async function () {
  ...
  it("Check marginal prices are correct for stable swap", async function () {
    const { pool, liquidityProvider, lpToken, toAtomicAmounts } =
        await loadFixture(testFixture);
    ...
    await pool.connect(liquidityProvider)
