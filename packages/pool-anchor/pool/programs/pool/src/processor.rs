...
use {
    crate::{
        ...
        error::PoolError,
        instruction::{
            DeFiInstruction, GovernanceInstruction, PoolInstruction, ProcessDefiInstruction,
...
pub mod Processor {
    ...
    fn process_init(
        ...
        let pool_authority_account =
            Self::get_pool_authority(pool_account.key, nonce, ctx.program_id)?;
        ...
        let token_decimals: [_; TOKEN_COUNT] =
            create_result_array(|i| -> Result<_, ProgramError> {
                let mint_decimals =
                    Self::check_program_owner_and_unpack::<MintState>(token_mint_accounts[i])?
                        .decimals;
    ...
    fn process_defi_instruction(
        ...
        let mut account_info_iter = accounts.iter();
        let pool_account = ctx.accounts.state;
        let mut pool_state = Self::check_and_deserialize_pool_state(pool_account, &ctx.program_id)?;
        ...
        if *pool_authority_account.key
            != Self::get_pool_authority(pool_account.key, pool_state.nonce, ctx.program_id)?
        {
    ...
    fn process_governance_instruction(
        ...
        let pool_account = ctx.accounts.state;
        let mut pool_state =
            Self::check_and_deserialize_pool_state(&pool_account, &ctx.program_id)?;

        Self::verify_governance_signature(ctx.accounts.governance_account, &pool_state)?;
        ...
        match governance_instruction {
            ...
            GovernanceInstruction::AdjustAmpFactor {
                ...
                pool_state.amp_factor.set_target(
                    Self::get_current_ts()?,
                    target_value,
                    target_ts,
                )?;
