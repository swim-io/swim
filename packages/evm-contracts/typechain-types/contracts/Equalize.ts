/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
  PromiseOrValue,
} from "../common";

export interface EqualizeInterface extends utils.Interface {
  functions: {
    "MAX_AMOUNT()": FunctionFragment;
  };

  getFunction(nameOrSignatureOrTopic: "MAX_AMOUNT"): FunctionFragment;

  encodeFunctionData(
    functionFragment: "MAX_AMOUNT",
    values?: undefined
  ): string;

  decodeFunctionResult(functionFragment: "MAX_AMOUNT", data: BytesLike): Result;

  events: {};
}

export interface Equalize extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: EqualizeInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    MAX_AMOUNT(overrides?: CallOverrides): Promise<[BigNumber]>;
  };

  MAX_AMOUNT(overrides?: CallOverrides): Promise<BigNumber>;

  callStatic: {
    MAX_AMOUNT(overrides?: CallOverrides): Promise<BigNumber>;
  };

  filters: {};

  estimateGas: {
    MAX_AMOUNT(overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    MAX_AMOUNT(overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
