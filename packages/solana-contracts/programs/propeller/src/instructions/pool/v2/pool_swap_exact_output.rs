...
use crate::{SwapExactOutputParams, Propeller, PropellerError, PropellerSender, TOKEN_COUNT, TRANSFER_NATIVE_WITH_PAYLOAD_INSTRUCTION, TransferWithPayloadData};
...
#[derive(Accounts)]
pub struct PoolSwapExactOutput<'info> {
	#[account(mut)]
...
impl<'info> PoolSwapExactOutput<'info> {
	pub fn accounts(ctx: &Context<PoolSwapExactOutput>) -> Result<()> {
		Ok(())
	}
}

pub fn handle_pool_swap_exact_output(
	ctx: Context<PoolSwapExactOutput>,
	swap_exact_output_params: SwapExactOutputParams,
) -> Result<()> {
	msg!("add_and_wormhole_transfer");
	handle_approve(&ctx, &swap_exact_output_params)?;
	
	let swap_output_amount = handle_swap_exact_output(
		&ctx, &swap_exact_output_params
	)?;
...
pub fn handle_approve(
	...
	require_gt!(
		swap_exact_output_params.maximum_input_amount,
		0 as u64,
...
pub fn handle_swap_exact_output(
	...
	let exact_input_amounts = [0, swap_exact_output_params.maximum_input_amount];
	let swap_defi_ix = two_pool::instruction::DeFiInstruction::SwapExactOutput{
		maximum_input_amount: swap_exact_output_params.maximum_input_amount,
		input_token_index: swap_exact_output_params.input_token_index,
		exact_output_amounts: swap_exact_output_params.exact_output_amounts,
	};
	...
	//TODO: don't really need CPI return data in the pool-repo for this call
	...
	// Ok(Vec::<u64>::deserialize(&mut data.as_slice())?)
	Ok(swap_exact_output_params.exact_output_amounts)
}

pub fn revoke(ctx: &Context<PoolSwapExactOutput>) -> Result<()> {
	msg!("[revoke]: revoke");
	token::revoke(
		CpiContext::new(
			ctx.accounts.token_program.to_account_info(),
