...
use crate::{Propeller, PropellerError, PropellerSender, SwapExactInputParams, TOKEN_COUNT, TRANSFER_NATIVE_WITH_PAYLOAD_INSTRUCTION, TransferWithPayloadData};
...
#[derive(Accounts)]
pub struct PoolSwapExactInput<'info> {
	#[account(mut)]
...
impl<'info> PoolSwapExactInput<'info> {
	pub fn accounts(ctx: &Context<PoolSwapExactInput>) -> Result<()> {
		Ok(())
	}
}
...
pub fn revoke(ctx: &Context<PoolSwapExactInput>) -> Result<()> {
	msg!("[revoke]: revoke");
	token::revoke(
		CpiContext::new(
			ctx.accounts.token_program.to_account_info(),
