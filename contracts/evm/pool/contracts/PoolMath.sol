...
function addRemove(
  ...
  for (uint i = 0; i < tokenCount; ++i) {
    ...
    updatedBalances[i] = Equalized.wrap(uint64(updatedBalance));
  ...
  if (totalFee != 0) {
    ...
    for (uint i = 0; i < tokenCount; ++i) {
      ...
      feeAdjustedBalances[i] = Equalized.wrap(uint64(feeAdjustedBalance));
    }
    ...
    governanceMintAmount = Equalized.wrap(uint64(_governanceMintAmount));
  }
  else {
  ...
  userLpAmount = Equalized.wrap(uint64(_userLpAmount));
}}
...
function swap(
  ...
  Equalized[] memory knownBalances = new Equalized[](tokenCount-1);
  if (isInput && totalFee != 0) {
    for (uint i = 0; i < knownBalances.length; ++i) {
      ...
      knownBalances[i] = Equalized.wrap(uint64(knownBalance));
    }
  }
  else {
    for (uint i = 0; i < knownBalances.length; ++i) {
  ...
  if (isInput) {
    ...
    updatedBalances[index] = Equalized.wrap(uint64(
  ...
  userTokenAmount = Equalized.wrap(uint64(_userTokenAmount));
...
function removeExactBurn(
  ...
  if (totalFee != 0) {
    ...
    _outputAmount =  baseAmount - feeAmount;
    ...
    poolBalances[outputIndex] = Equalized.wrap(uint64(baseBalance - _outputAmount));
    ...
    uint _governanceMintAmount = governanceDepth * updatedLpSupply / totalLpDepth;
    governanceMintAmount = Equalized.wrap(uint64(_governanceMintAmount));
  }
  else {
    _outputAmount = baseAmount;
  }
  outputAmount = Equalized.wrap(uint64(_outputAmount));
}
