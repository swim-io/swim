...
export const msleep = (milliSec: number) =>
...
export async function ensureAccountIsFound(
  connection: Connection,
  key: PublicKey,
) {
  for (let i = 0; i < 60; ++i) {
    if (await connection.getAccountInfo(key)) return;
    await sleep(1);
  }
  throw new Error(
    `still couldn't find account for key ${key}, after waiting for a decent while`,
  );
}
