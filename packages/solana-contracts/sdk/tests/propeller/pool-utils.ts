...
import {array, bool, i64, publicKey, struct, u128, u32, u64, u8,} from "@project-serum/borsh";

//TODO: update
export const TWO_POOL_PROGRAM_ID = new PublicKey("SWimmSE5hgWsEruwPBLBVAFi3KyVfe8URU2pb4w7GZs");
...
export async function initalizeTwoPool(
	...
	const poolMint0Info = await provider.connection.getAccountInfo(poolMint_0.publicKey);

	if(!poolMint0Info){
		const initPoolMintTxn = await splToken
			.methods
	...
	const poolMint1Info = await provider.connection.getAccountInfo(poolMint_1.publicKey);
	if(!poolMint1Info){
		const initPoolMintTxn1 = await splToken
			.methods
			.initializeMint(mintDecimals, poolTokenMintAuth, null)
			.accounts({
	...
	const initLpMintTxn = await splToken
		.methods
		...
		.preInstructions([
			await splToken.account.mint.createInstruction(lpMint),
		])
		.signers([lpMint])
		.rpc();
	let blockConfirmStrategy = {
...
export async function initalizeTwoPoolV2(
	...
	const [poolPda, bump] = await PublicKey.findProgramAddress(
		[lpMint.publicKey.toBuffer()], TWO_POOL_PROGRAM_ID
	);
	...
	const poolLpMintAuth = poolAuth;
	const poolMint0Info = await provider.connection.getAccountInfo(poolMintInfo_0.mint.publicKey);
	...
	const poolMint1Info = await provider.connection.getAccountInfo(poolMintInfo_1.mint.publicKey);
	if(!poolMint1Info){
		const initPoolMintTxn1 = await splToken
			.methods
